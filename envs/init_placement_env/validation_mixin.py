...
class CatanValidationMixin:
    ...
    def __is_valid_settlement_placement(self, node_id):
        if self.observation_space["adj_is_built"][node_id].any():
            for adj_node_id in NODES_TO_NODES[node_id]:
                if self.observation_space["adj_is_built"][adj_node_id].any():
                    return False 
    ...
    def __is_valid_road_placement(self, edge_id, player):
        # TODO Fix this func
        return self.observation_space["edges_is_built"][:, edge_id].sum() == 0
